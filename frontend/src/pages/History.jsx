...
const History = () => {
  ...
  const openViewer = async (item, type) => {
    try {
      if (type === 'notes') {
        // For notes, we already have the content in the history item
        setSelectedItem(item);
        setViewerType('notes');
      } else if (type === 'flashcards') {
        // Fetch full flashcard set data
        const response = await fetch(`http://localhost:5000/api/flashcards/${item.contentId}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
        if (response.ok) {
          const data = await response.json();
          setSelectedItem(data.flashcard);
          setViewerType('flashcards');
        }
      } else if (type === 'quizzes') {
        // Fetch full quiz data
        const response = await fetch(`http://localhost:5000/api/quizzes/${item.contentId}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
        if (response.ok) {
          const data = await response.json();
          setSelectedItem(data.quiz);
          setViewerType('quizzes');
        }
      }
    } catch (err) {
  ...
  const renderHistoryItem = (item, type) => {
    ...
    return (
      <div key={item._id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-4 mb-4 cursor-pointer hover:shadow-lg transition-shadow" onClick={() => openViewer(item, type)}>
        <div className="flex items-start justify-between">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              deleteHistoryItem(type, item._id);
            }}
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {/* Viewers */}
      {selectedItem && viewerType === 'notes' && (
        <NotesViewer note={selectedItem} onClose={closeViewer} />
      )}
      {selectedItem && viewerType === 'flashcards' && (
        <FlashcardsViewer flashcardSet={selectedItem} onClose={closeViewer} />
      )}
      {selectedItem && viewerType === 'quizzes' && (
        <QuizViewer quiz={selectedItem} onClose={closeViewer} />
      )}
    </div>
  );
};
