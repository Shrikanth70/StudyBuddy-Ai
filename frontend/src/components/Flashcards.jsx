        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.success && data.flashcards) {
                        ...
                        if (saveResponse.ok) {
                            ...
                            try {
                                await fetch('http://localhost:5000/api/history', {
                                    ...
                                    body: JSON.stringify({
                                        type: 'flashcards',
                                        contentId: saveResponse.data.flashcard?._id || saveResponse.data.data?._id,
                                        title: title || `AI Generated Flashcards on ${topic}`,
                                        topic: topic || title,
                                        subject: subject,
                    ...
                    // Use fallback flashcards if AI failed
                    setCards(data.fallback);
                    setNumCards(data.fallback.length);
                    setSuccess('Flashcards generated using fallback method!');

                    // Still try to save fallback flashcards
                    try {
                        const saveResponse = await fetch('http://localhost:5000/api/flashcards', {
                            method: 'POST',
                            headers: {
                            ...
                            body: JSON.stringify({
                                title: title || `Flashcards on ${topic}`,
                                subject,
                                topic: topic || title,
                                difficulty,
                                cards: data.fallback,
                        ...
                        if (saveResponse.ok) {
                            setSuccess('Flashcards generated and saved to your library!');
                        } else {
                            setSuccess('Flashcards generated successfully! (Note: Auto-save failed, please save manually)');
                        }
                    } catch (saveErr) {
                        console.error('Auto-save failed:', saveErr);
                        setSuccess('Flashcards generated successfully! (Note: Auto-save failed, please save manually)');
