    ...
    const generateNotes = async () => {
        ...
        try {
            ...
            if (data.success && data.notes) {
                    ...
                    if (saveResponse.status === 201 || saveResponse.data.success) {
                        ...
                        try {
                            await fetch('http://localhost:5000/api/history', {
                                ...
                                body: JSON.stringify({
                                    type: 'notes',
                                    contentId: saveResponse.data.note?._id || saveResponse.data.data?._id,
                                    title: `Notes on ${topic}`,
                                    topic: topic,
                                    subject: subject || data.metadata?.subject || 'General',
                                    content: data.notes,
                ...
                // Use fallback notes if AI failed
                setGeneratedNotes(data.fallback);
                if (!subject && data.metadata && data.metadata.subject) {
                    setSubject(data.metadata.subject);
                }
                setSuccess('Notes generated using fallback method!');

                // Still try to save fallback notes
                try {
                    const saveResponse = await api.post('/study-notes', {
                        title: `Notes on ${topic}`,
